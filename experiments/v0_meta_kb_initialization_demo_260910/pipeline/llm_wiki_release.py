...
def pipeline(pages: list[dict[str, Any]], claims: list[dict[str, Any]], sources: list[dict[str, Any]], snapshot: dict[str, Any], config: dict[str, Any], build: str, at: str, m: dict[str, Any]) -> None:
    ...
    ywrite(EXP / "00_inputs/page_plan.yaml", {"plan_id": f"page-plan:{h(build)}", "build_id": build, "generated_at": at, "strategy": "question-oriented, claim-backed, typed-link compilation", "pages": [{"uid": p["uid"], "path": p["path"], "page_type": p["type"], "purpose": p["summary"], "claim_refs": p["claims"], "source_refs": p["sources"], "planned_links": p["outgoing"]} for p in pages]})
    questions = [
