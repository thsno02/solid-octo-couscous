...
def write_pages(pages: list[dict[str, Any]], build: str, at: str, sources: list[dict[str, Any]]) -> None:
    ...
    for p in pages:
        ...
        front.update(
            {
                "rendered_claim_refs": p["rendered_claims"],
                "rights_refs": p["rights_refs"],
                "rights_unavailable_source_refs": p["rights_unavailable_source_refs"],
            }
        )
...
def machine(pages: list[dict[str, Any]], claims: list[dict[str, Any]], evidence: list[dict[str, Any]], sources: list[dict[str, Any]], build: str, at: str) -> dict[str, Any]:
    ...
    jwrite(WIKI / "catalog/pages.jsonl", ({"uid": p["uid"], "title": p["title"], "path": p["path"], "page_type": p["type"], "summary": p["summary"], "claim_refs": p["claims"], "rendered_claim_refs": p["rendered_claims"], "source_refs": p["sources"], "rights_refs": p["rights_refs"], "rights_unavailable_source_refs": p["rights_unavailable_source_refs"]} for p in pages))
    jwrite(WIKI / "catalog/claims.jsonl", ({"uid": str(c["uid"]), "text": ctext(c), "scope": cscope(c), "domain": cdomain(c), "source_refs": srcs(c), "evidence_refs": evrefs(c), "rights_refs": object_rights_refs(c), "rights_unavailable_source_refs": object_unavailable_sources(c), "page": cpath[str(c["uid"])]} for c in claims))
    jwrite(WIKI / "catalog/sources.jsonl", ({"uid": str(s["uid"]), "title": text(s.get("title")), "source_type": s.get("source_type"), "content_tier": s.get("content_tier"), "revision": s.get("revision"), "rights_status": s.get("rights_status"), "rights": source_rights(s), "page": spath[str(s["uid"])]} for s in sources))
    nodes = [{"id": p["uid"], "kind": "page", "path": p["path"], "label": p["title"], "rights_refs": p["rights_refs"], "rights_unavailable_source_refs": p["rights_unavailable_source_refs"]} for p in pages] + [{"id": str(c["uid"]), "kind": "claim", "label": ctext(c)[:180], "rights_refs": object_rights_refs(c), "rights_unavailable_source_refs": object_unavailable_sources(c)} for c in claims] + [{"id": str(e["uid"]), "kind": "evidence", "label": text(prop(e).get("selector")), "rights_refs": object_rights_refs(e), "rights_unavailable_source_refs": object_unavailable_sources(e)} for e in evidence] + [{"id": str(s["uid"]), "kind": "source", "label": text(s.get("title")), "rights_status": s.get("rights_status"), "rights": source_rights(s)} for s in sources]
    ...
    ywrite(WIKI / "graph/dependency_graph.yaml", {"build_id": build, "generated_at": at, "nodes": len(nodes), "edges": len(edges), "page_dependencies": {p["uid"]: {"path": p["path"], "claim_refs": p["claims"], "rendered_claim_refs": p["rendered_claims"], "source_refs": p["sources"], "rights_refs": p["rights_refs"], "rights_unavailable_source_refs": p["rights_unavailable_source_refs"], "page_refs": [x["target"] for x in p["outgoing"]]} for p in pages}})
    jwrite(WIKI / "search/page_index.jsonl", ({"uid": p["uid"], "path": p["path"], "title": p["title"], "page_type": p["type"], "summary": p["summary"], "rights_refs": p["rights_refs"], "rights_unavailable_source_refs": p["rights_unavailable_source_refs"], "search_text": text(p["title"] + " " + p["summary"] + " " + re.sub(r"[`#>*|]", " ", p["body"]))[:6000]} for p in pages))
    packs = [
