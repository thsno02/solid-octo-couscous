...
def build_atomic_pages(sources: list[dict[str, Any]], claims: list[dict[str, Any]], evidence: list[dict[str, Any]], snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    for uid, claim in sorted(by_uid.items()):
        ...
        p["claims"] = [uid]; p["sources"] = refs; p["sections"] = [{"heading": "Candidate statement", "claim_refs": [uid], "source_refs": refs, "editorial_intent": "Expose the exact candidate statement."}, {"heading": "Evidence bindings", "claim_refs": [uid], "source_refs": refs, "editorial_intent": "Resolve to local selectors."}]
        for ref in refs:
    ...
    for uid, row in sorted(srow.items(), key=lambda item: titles[item[0]].lower()):
        ...
        p["claims"] = [str(x["uid"]) for x in rows]; p["sources"] = [uid]; p["sections"] = [{"heading": "Source-reported candidate statements", "claim_refs": [str(x["uid"]) for x in rep], "source_refs": [uid], "editorial_intent": "Represent source statements."}, {"heading": "Collection assessments", "claim_refs": [str(x["uid"]) for x in assess], "source_refs": [uid], "editorial_intent": "Keep collector interpretation separate."}]
