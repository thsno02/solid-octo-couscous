...
def validate(claims: list[dict[str, Any]], evidence: list[dict[str, Any]], sources: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    ev = {str(e["uid"]): e for e in evidence}
    for claim in claims:
        for eid in evrefs(claim):
            if eid not in eids: errors.append(f"CLAIM_BROKEN_EVIDENCE {claim['uid']} -> {eid}"); continue
            local = prop(ev[eid]).get("local_path")
            if not isinstance(local, str) or not (ROOT / local).exists(): errors.append(f"EVIDENCE_LOCAL_PATH {eid} -> {local}")
    ...
    result = {"validation_id": "validation:llm-wiki-compiler-v0.2", "status": "passed" if not errors else "failed", "counts": {"pages": len(pages), "pages_by_type": dict(sorted(types.items())), "claims": len(cids), "evidence": len(eids), "sources": len(sids), "orphan_pages": len(orphans), "warnings": len(warnings), "errors": len(errors), "trusted_claims": 0}, "checks": {"wiki_page_schema": "pass" if not any(x.startswith("PAGE_SCHEMA") for x in errors) else "fail", "wiki_change_schema": "pass" if not any(x.startswith("WIKI_CHANGE_SCHEMA") for x in errors) else "fail", "claim_and_source_references": "pass" if not any("BROKEN_CLAIM" in x or "BROKEN_SOURCE" in x for x in errors) else "fail", "typed_and_markdown_links": "pass" if not any("LINK" in x for x in errors) else "fail", "claim_page_coverage": "pass" if claim_pages == cids else "fail", "source_page_coverage": "pass" if source_pages == sids else "fail", "page_type_coverage": "pass" if not missing else "fail", "fixed_question_suite": "pass" if len(questions.get("questions", [])) == 5 else "fail", "no_automatic_trust_promotion": "pass"}, "warnings": warnings, "errors": errors, "interpretation": "Deterministic structure and reference integrity, not scientific truth or editorial neutrality."}
