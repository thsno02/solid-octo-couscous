...
该文件头额外写明分发时须附原 `natbib.dtx`，不能只把普通 LPPL 1.3c §3 当作撤销这项具体声明的依据。此前取得的历史小包实际是 8.31a；本轮从 [Debian 的 2009 历史源码归档](https://sources.debian.org/data/main/t/texlive-base/2009-11%2Bsqueeze1/texmf-dist/source/latex/natbib/natbib.dtx)取得真正的 248,383-byte、2009/07/16 8.31 原源码。用原件内置 installer，在临时目录以 `tex -no-shell-escape` 和 `package,all` 生成 45,154-byte STY，与三篇 AI 论文、ADAS 及 Self-RAG 共五份实存组件逐字节一致，包括文件头。没有下载整个 TeX 发行版，也没有修改旧组件或论文。

原源码许可为 LPPL1-or-later，未另要求全发行包或每个胶囊必须同目录保存。整个 repo 分发时实际随附共享的原源码 `raw_data/licenses/components/natbib-8.31/natbib.dtx`，保留出处、原版权和适用 LPPL；四项各自组合 NOTICE 也保存完整原源码代码块，无需新建来源特例或组件管理框架。该精确生成结果用于本次来源对应，不新增所有 LPPL 文件都需重新编译的法律门槛。`fancyhdr` 没有同一源码声明，不能将 natbib 条件扩张到它。

扩大到本轮起点 `064c504` 的已提交组件清单后，17 个胶囊中的 18 份 natbib.sty（PBT 有两份）均与同一生成件相同，具体路径记录在机器审计的 `shared_component_findings`。这解决一个可复用的组件问题，不使这 17 篇论文自动获得全文放行。Darwin Gödel Machine 后续已对应固定 v3、核定模板表达的许可路径并完成当前包履约；其余来源仍须分别核对实际条件，不再以该 natbib 原源码不可得为现时阻碍。
...
ADAS 的[固定 arXiv v2](https://arxiv.org/abs/2408.08435v2)源响应为 241,563 bytes，与既有 revision 和全部九个实存 source 成员对应，论文 CC BY4 已核实。[ICLR2025 专用官方模板](https://www.overleaf.com/latex/templates/template-for-iclr-2025-conference-submission/gqzkdyycxtvt)列 Author ICLR 和 CC BY4；实存 9,025-byte STY 与[官方固定提交](https://github.com/ICLR/Master-Template/blob/05833d63fe48bbf250b144741ea77691018bb328/iclr2025/iclr2025_conference.sty)逐字节一致，保留 Hugo Larochelle 的改编归属。12,284-byte `math_commands.tex` 也与同一提交对应，另保留 [goodfeli/dlbook_notation 原作者自由使用说明](https://github.com/goodfeli/dlbook_notation/blob/master/README.md)和来源，不虚构 MIT/SPDX 声明或书籍全文许可。BST、fancyhdr 与 natbib 各保持原 LPPL 条件，不被论文或模板 BY4 替代。

四项的六字段许可包、完整法律文本、组件归属和实际文末附注现已全部落实，当前固定包均为 `allow`：AI Scientist v3、ADAS v2、Agent Laboratory v2、AI Scientist-v2 v1。两次真实固定版本重建逐字节一致；分别保留 26／9／8／17 份 source 与 60／33／84／78 个 selectors，规范化 TeX 完全不变，原正文完整前缀保留，仅追加唯一许可末注，没有删除正文、参考材料或组件。最终独立复核修正三个 AI 组合 NOTICE 中各三句旧模板对象名称残留，法条和源码块不动；随后各做两次离线 finalize，全文件幂等，不称为再次网络重建。许可声明沿现有通用路径传播；该批集成 build 为 `b04c61704bd4b7be`。这些局部放行不替代剩余来源许可门，也不构成人工批准或整体 PR 完成。

### DGM、Self-RAG 与 OpenScholar：当前公开许可对应实存表达

[DGM 固定 v3](https://arxiv.org/abs/2505.22954v3)、[Self-RAG 固定 v1](https://arxiv.org/abs/2310.11511v1)及 [OpenScholar 固定 v1](https://arxiv.org/abs/2411.14199v1)的官方当前文章许可均为 BY4，实际源响应分别为 2,294,917、917,558、2,758,337 bytes，与各自既有 revision 和实存文字成员对应。三项现已按当前固定包条件全部履约，gate=`allow`。

模板授权（template authorization）依赖 [ICLR 官方2025模板的公开 BY4 声明](https://www.overleaf.com/latex/templates/template-for-iclr-2025-conference-submission/gqzkdyycxtvt)，并逐文件核对[固定2025 STY](https://github.com/ICLR/Master-Template/blob/05833d63fe48bbf250b144741ea77691018bb328/iclr2025/iclr2025_conference.sty)与实际保留表达，而不是按相邻年份泛化。DGM 的[官方2026复制提交](https://github.com/ICLR/Master-Template/commit/067b60c3985bd549c8b1afabb426a007905cea46)只改两处年号，本地再有作者表格 `l→c` 一处差异；Self-RAG 保留 Preprint 页眉、2024 review 年号和已注释停用的竖尺等既存差异；OpenScholar 保留额外注释行及 `Asai et al. (2024)` 页眉。其余对应表达已实质逐文件核对，归属和差异随包记录。这里使用当前公众授权及具体改编/表达对应，不虚构独立历史2024或2026许可，不声称旧原件来自2025，也不编造修改者或原提交时已取得该授权。

三个包的 BST、fancyhdr、natbib 分别保留原 LPPL1-or-later、本包选择1.3c；完整法律文本和匹配原 natbib.dtx 随 NOTICE 与 repo 保存。Self-RAG 的 notation 另保留原作者自由使用声明，不套论文 BY4 或虚构 SPDX。Self-RAG 实存三份 ACL 摘要分别对应 [E-BERT](https://aclanthology.org/2020.findings-emnlp.71/)、[Open-Domain Question Answering](https://aclanthology.org/2020.acl-tutorials.8/)和 [ELI5](https://aclanthology.org/P19-1346/)；OpenScholar 的五份包括这三份及 [The Dawn After the Dark](https://aclanthology.org/2024.acl-long.586/)、[Improving Text Embeddings](https://aclanthology.org/2024.acl-long.642/)。它们按[ACL作品级 BY4 政策](https://aclanthology.org/faq/copyright/)分别保留完整作者、作品、年份和来源，不作为论文作者原创或 metadata CC0，不外推所引作品全文、模型或数据。DGM 编译元数据单列 arXiv CC0。

三项各完成两次真实固定版本重建和两次 finalize 幂等验证，全胶囊字节一致；分别保留 34／15／16 份 source 和 77／69／110 个 selectors，规范化 TeX 与原正文完整前缀不变，只加唯一末注，没有删除正文、摘要或组件。许可沿现有通用路径传播，未添加来源特例或放宽门控；23 项局部放行仍不等于整库或 PR 发布批准。

### GraphRAG 与 DiscoveryWorld：模板对应表达与独立短引边界

GraphRAG 固定 v2 的论文许可为 CC BY 4.0，DiscoveryWorld 固定 v2 为 CC BY-SA 4.0；两者均已由固定源响应对应到实际保存的文字成员。当前 [NeurIPS 官方模板](https://www.overleaf.com/latex/templates/formatting-instructions-for-neurips-2026/bjdwqfdkyftc) 明确列 NeurIPS 2026 Program Chairs、CC BY 4.0；[官方 ZIP](https://media.neurips.cc/Conferences/NeurIPS2026/Formatting_Instructions_For_NeurIPS_2026.zip) 为 20,259 bytes，含 13,704-byte STY、模板 TeX 和 checklist。

本轮核对的是获许可表达（licensed expression），不是从文件名推定历史许可。两个旧 STY 中页面几何、字体、标题、列表、摘要及行号兼容代码的对应表达已逐块核对；年号、会议事实、短页脚、匿名开关和回答宏等既存差异分别披露。DiscoveryWorld 保留 Lora Aroyo 与 Roman Garnett 等原始归属；GraphRAG 的 15 项 checklist 与当前指南对应，作者填写的回答独立归入论文范围。当前模板新增的 track、workshop、education 等功能不冒称旧件所有。此路径不宣称原文件来自 2026，也不虚构历史 BY4 或未知修改者。
...
### PR26 当前单页响应（Dated Response）

本批固定 `dated-response-2026-09-17T14:20:17Z`：批准的同址 GET 于 `2026-09-17T14:20:15Z–14:20:17Z` 返回 HTTP200、`text/html; charset=utf-8`，无重定向／Content-Encoding。55,141-byte 原 HTML 的 revision 仍为 `93a9f00b0006695dab8f1bde801a40e36eed8c236c4c839aff645bec38b5dc6c`；相同 revision 不意味着旧 prose-only 包已经包含原响应。

当前有界组合为 **Apache-2.0 AND MIT**：LinkML 作者内容仍依下述文档 Apache 证据；实际响应标明的 Furo 2025.12.19 内嵌模板、CSS、theme 初始化和 SVG 依其[官方 MIT LICENSE](https://raw.githubusercontent.com/pradyunsg/furo/2025.12.19/LICENSE)与同 tag 的实际匹配模板。新[专属载体](../raw_data/licenses/linkml-page-body-02-260917.md)保留完整 Apache／MIT、LinkML Authors 2021–2026 与 Pradyun Gedam 2020 版权、Sphinx／Furo 署名及 Just the Docs／Feather／Tabler 来源注记；不声称独立审核全部图标上游链，也不把根许可当第三方权利担保。AND 是分组件履约，不是任选 OR。

原 HTML 不改；派生仅消费 `article#furo-main-content`，去除 `a.headerlink` 的 ¶ UI，保留作者目录文字、URI／CURIE 和链接目标。内容审读发现 `li/p/blockquote` 的部分嵌套层次及末尾索引列表在 Markdown 中未正确保留：原 L557–565 对应派生 L494–532，派生 L771–789 亦有空列表标记，故文本提取明确为 partial，原 HTML 可复查结构；不声称保真转换完成。source 行范围是包围溯源区间，不证明整份响应逐行转换。旧 root document／164 selectors 原字节保留，旧 NOTICE 另存 capsule 的 `history/NOTICE-before-page-body-02.md`，完整旧 C／manifest／R 审阅作为历史保全。本页不是完整 LinkML 方法文档；外部 Logo、脚本／样式、其他页面及外链作品未下载，不作其准入或离线站点功能完整声明。实际四方包、重放及未完成项以 [PR26 阶段报告](plans/260916-corpus-completion-main-convergence/p3-page-body-02.md)为准，不据本段替代最终门控。

### PR26 之前的文字包记录（Historical Text-only Package）

下列原记录中的“当前／本次”指旧文字包检查点，不是本次完整原响应范围：
...
本批固定 `dated-response-2026-09-17T14:20:15Z`：批准的同址 GET 开始／完成均在该 UTC 秒，HTTP200、`text/html`、`Content-Encoding: gzip`，无重定向。原 wire 为 4,500 bytes，机械解压 HTML entity 为 12,007 bytes；同一次响应不是两次 GET。entity revision 是 `b5be08b7c47762f190c539e7dcf6ba04c60507c310af960578f2f97623ee5a0d`，虽长度相同却不等于旧 `f62a65…`；两层分别保留并核实际解压等式，不挪用 wire 长度或旧 grant。

当前有界组合为 **CC-BY-SA-3.0 AND Apache-2.0**：官方 [FAQ #18](https://schema.org/docs/faq.html#18)将 supporting documentation／示例与网站软件分开；实际内嵌 header SVG／事件属性和 GA loader 与[官方软件仓库](https://github.com/schemaorg/schemaorg)模板对应，依其 Apache LICENSE 处理。新[专属载体](../raw_data/licenses/schemaorg-page-body-02-260917.md)包含完整 BY-SA／Apache、四 Sponsors 归属、来源、修改与范围说明。没有把远程 `analytics.js`、jQuery、服务或仅被链接的作品纳入本站软件授权，也不把移动的模板 `main` 路径伪称网页部署 commit。

派生仅消费 `article#mainContent`，保留旧 helper 漏掉的未包裹正文、dated 数量、层次、链接与 pending／meta／attic 警示；TermFinder 静态界面不作为事实 claim，未取得交互结果。文档改编与对应 selector 摘录按 BY-SA 3.0 履行同许可共享；独立 Apache 模板及集合内其他作品不被强行重许可。旧 root document／34 selectors、旧完整 C／manifest／R 与 acquisition 事实保全，旧 NOTICE 另存 `history/NOTICE-before-page-body-02.md`。实际 development／V30.1 页脚不是已取得完整 Vocabulary release 的证明；本页总体 UID 边界仍未决，重放和最终门控见 [PR26 阶段报告](plans/260916-corpus-completion-main-convergence/p3-page-body-02.md)。

### PR26 之前的文字包记录（Historical Text-only Package）

下列“当前包／本项”指旧快照及旧 BY-SA 文字范围，原记录保留，不代表本次新响应：
...
- GraphRAG 的固定 v2 先前待核模板条件已按上节对应表达路径和实际差异完成包装，当前固定文字包为 `allow`；Fed 短引保持独立情境边界。匿名投稿页脚中的 “Do not distribute” 从未被当作样式本身禁止分发的条款。
...
- Progressive Neural Networks 实存 March 2016 模板尚缺覆盖实际版本的公众许可依据，仍为 `block`。Darwin Gödel Machine 已按上节固定 v3、具体模板表达对应与组件条件完成包装，当前包为 `allow`；这不自动放行其他模板或论文，也不将投稿页脚的 Do not distribute 当作模板禁令。
...
## 论文按实际组件分组与履约状态（Component-family triage）

前期实读九个起初待办的包，将通用“条件尚待证明”拆成具体差异；其中 ADAS 固定 v2、Self-RAG 与 OpenScholar 各固定 v1，以及本轮 DiscoveryWorld 固定 v2 已完成包装并放行，其余项目不自动继承。未保存的依赖或图片不新增清权要求，论文自身摘要不与书目中复制的第三方摘要混算。

| 来源组 | 已查明的具体范围 | 下一步边界 |
|---|---|---|
| ADAS / ICLR2025 | 固定v2与9个源文件对应；书目无额外摘要。ICLR2025 STY明确BY4，notation保留原作者使用说明；BST/fancyhdr/natbib独立LPPL，完整原dtx实际随包及repo保存 | 当前固定文本包已包装 gate=allow；原source/33selectors/正文前缀不变，不外推其他年份 |
| Self-RAG / OpenScholar | 固定v1各保留15／16源文件、69／110selectors及3／5个ACL作品摘要；实际STY表达已对应当前官方2025 BY4，既存差异披露，LPPL/原dtx分别履约 | 两个当前固定文本包已包装 gate=allow；不虚构历史许可或旧件来自2025，不删除摘要或组件 |
| Fast Model Editing / Memory-Based Editing | 保存11／18个第三方摘要；前者ICLR2022，后者ICML2022及algorithm组件 | 混合出版来源不能一律套ACL许可；前者没有实存natbib，不增加该随附要求 |
| MLE-bench | 22个摘要含论文及博客；额外arxiv.sty是旧ICLR派生，不是George Kour MIT模板 | 不能作为纯ICLR2025四组件包直接放行 |
| WikiGenBench | 两份不同STY，其中acl2023内部标ACL2020；另存DOI10.1121/1.2016299完整摘要 | 文件名不证明许可；两模板与该具体摘要分别对应 |
| AgentRxiv | GDM自身BY-SA4；唯一额外摘要LUMI-lab的固定bioRxiv v1为BY-NC-ND4 | 本论文BY4不能覆盖摘要，实际用途与转换条件仍须核实 |
| DiscoveryWorld | 固定v2原响应、9个文字成员及正文对应；论文BYSA4，11416-byte样式的对应表达与当前官方BY4模板逐块核对 | 当前固定包已包装 gate=allow；披露既存短适配，不声称历史2024整件BY4，不将两组件许可扩张到整库 |

[ICLR2025 专用官方模板](https://www.overleaf.com/latex/templates/template-for-iclr-2025-conference-submission/gqzkdyycxtvt)明确署名 ICLR、CC BY4 及官方源码目录；ADAS 的 9,025-byte STY 与[新增2025模板的固定文件](https://github.com/ICLR/Master-Template/blob/05833d63fe48bbf250b144741ea77691018bb328/iclr2025/iclr2025_conference.sty)完全相同。[DiscoveryWorld 使用的官方赛道样式包](https://media.neurips.cc/Conferences/NeurIPS2024/NeurIPS-Dataset-Styles.zip)对应证据只解决组件身份，不能单独替代缺少的许可依据。

## 已对应的替代出版路径（Alternate publication routes）

arXiv 的 nonexclusive 只说明该平台得到的托管权，不足以排除同一表达在正式出版物中的其他许可。三项固定源响应及保留成员已与现存 revision 对应，作者正式出版正文及附录的 BY4 路径现已闭合：

- RE-Net：[固定 v4 源包](https://export.arxiv.org/e-print/1904.05530v4)为 1,052,179 bytes，12 个实存 source 全部同字节，另 30 个 tar 成员原未保存。[EMNLP 2020 正式作品](https://aclanthology.org/2020.emnlp-main.541/)为 15 页，正式题名用单数 **Recurrent Event Network**；正文 1–6、表 1–4、图 caption 1–8、附录 A–G 对应，PDF 全文 tokens 仅页码/页眉和 arXiv stamp 不同。剩余缺口是源内旧摘要、禁用旧表和 OtherRelatedWork 的完整旧稿表达未证明属于已许可出版范围，不对数字事实另设授权要求；STY 主排版范围及嵌入 everyshi/eso-pic、BST 的独立 LPPL 条件仍待履约。
- RARR：[固定 v3 源包](https://export.arxiv.org/e-print/2210.08726v3)为 988,552 bytes，33 个实存 source 共 214,711 bytes 全同字节，115,974-byte 正文及 101 selectors 已绑定。[ACL 2023 正式作品](https://aclanthology.org/2023.acl-long.910/)为 32 页；主文、附录 A–E（`7a_human_eval` 对应附录 C）及 `fig_prompts` 的 28 个实质 evidence 行均对应。剩余条件集中在已定位 Wikipedia、新闻及旅游等独立摘录的有限集合，以及 STY 两处 EMNLP→ACL 标签差异和 BST LPPL 包装；科学研究引用需按具体表达判断，不一概要求普通事实句或短引另行授权。
- ResearchAgent：[固定 v2 源包](https://arxiv.org/src/2404.07738v2)为 8,125,839 bytes，33 个实存 source 共 282,197 bytes 全同字节。[NAACL 2025 正式作品](https://aclanthology.org/2025.naacl-long.342/)为 30 页，主文、附录 A/B 及 16 个表对应，未见额外未出版主体。剩余具体条件是 [Vamathevan 2019](https://www.nature.com/articles/s41573-019-0024-5)额外完整摘要和 FlyWire 固定预印本输入摘要尚缺适用依据，两份 ACL 输入摘要已有独立 BY4 路径但尚待包装；STY 与当前官方模板有真实差异，不能称同字节，Goodfeli 使用声明及 BST LPPL 也分别履约。Nature 的 permissions 入口不等于禁止复制，后出版改写摘要的许可也不能自动追溯旧表达。

[ACL 版权政策](https://aclanthology.org/faq/copyright/)对 2016 年后的本机构材料提供公众 BY4。按既有判定模型，三项均为 `documented_permission`、`CC-BY-4.0`、`allow_after_conditions_met`，但当前 gate 仍为 `block`：作者出版许可不等于全部 tar、未出版旧稿或独立摘录/模板获得重新授权。该轮只更新三项审计及汇总，未改这三项的正文、source、metadata 或派生产物，也未自动删除或整理已识别材料；许可未核实不等于禁止，不新增作者另函或人工 approval 门槛。

另完成四项有界清单核查：MEMORYLLM 的两个书目文件没有摘要；PaperBench 的 26 个 abstract 字段实际含 2 个空值、4 个短描述/截断占位和 20 个较长摘要，不一概称完整作品。GraphRAG 固定 v2 的 6,473,933-byte 响应与既有 revision、19 个保留源成员一致；其 NeurIPS2024 样式及 checklist 已按当前授权表达的实质对应及既存差异完成包装，不以已更新到 2026 的模板入口 URL 单独证明历史许可。Open-Endedness 实存 DeepMind 模板独立 BY-SA4，不能由主论文 BY4 覆盖所有组件。具体文件事实保存在机器审计；本轮不删除正文、模板或书目表达。

## 来源族发现（Source-family Findings）

- arXiv：32 篇官方文章页链接 CC；WikiChat 另由作者的确切 camera-ready 版本声明与 ACL 出版许可形成正文授权链；RE-Net、RARR、ResearchAgent 另由固定 payload 与正式 ACL 出版表达对应形成作者正文 BY4 路径。其余 37 篇仍缺适用公众许可证据；arXiv non-exclusive/历史 assumed license 本身不授予公众再分发权，正文许可不自动覆盖所有附带组件。
- W3C：7 份页面采用可修改的 Software and Document License；7 份旧页面采用限制一般衍生的 Document License。
- Apache/LinkML/ODCS：ODCS 固定定义首页沿既有 Apache-2.0 范围；LinkML 本次 dated 原响应分别对作者内容与内嵌 Furo 模板履行 Apache-2.0 AND MIT，旧 prose-only 记录保全；范围不扩展到各自整个站点。
- Schema.org：本次 dated 响应分别对 supporting documentation 与内嵌软件履行 CC-BY-SA-3.0 AND Apache-2.0，文档适配物保留 SA、旧 text-only 历史不覆盖；未来新版本仍需重新核验。
...
| 5 | <code>arxiv-1904.05530</code> | <code>materialized_sources/corpus/arxiv-1904.05530--cddfa770/manifest.yaml</code><br><code>raw_data/arxiv/Recurrent Event Network for Reasoning over Temporal Knowledge Graphs/metadata.yaml</code> | 固定 v4 与 12 个 source 同字节；EMNLP2020 作者正文、表图文字及附录表达 BY4 路径已对应 | 源内三类完整旧稿表达的许可范围、STY 主排版及独立 LPPL 条件、当前包 NOTICE 尚待落实 | <code>documented_permission</code> | 满足具体剩余条件后再发布；当前 gate=block，不泛称主文许可未证 |
...
| 23 | <code>arxiv:2210.08726</code> | <code>materialized_sources/corpus/arxiv-2210.08726--8e79abf3/manifest.yaml</code><br><code>raw_data/arxiv/RARR: Researching and Revising What Language Models Say, Using Language Models/metadata.yaml</code> | 固定 v3 与 33 个 source 同字节；ACL2023 作者主文及附录 A–E 表达 BY4 路径已对应 | 具体独立摘录的有限集合依据、STY 标签改编/BST 条件和 NOTICE 尚待落实；事实句、普通短引不一概另行授权 | <code>documented_permission</code> | 满足具体剩余条件后再发布；当前 gate=block，不重新许可第三方表达 |
...
| 30 | <code>arxiv:2310.11511</code> | <code>materialized_sources/corpus/arxiv-2310.11511--2a23ab40/manifest.yaml</code><br><code>raw_data/arxiv/Self-RAG: Learning to Retrieve, Generate, and Critique through Self-Reflection/metadata.yaml</code> | 固定 v1；论文 BY4、实际STY对应当前2025公开BY4、三篇ACL摘要BY4、notation原作者使用说明及独立LPPL/原dtx分别履约 | 无；15源文件、69selectors、TeX与原正文前缀不变，既存模板差异已披露 | <code>documented_permission</code> | 当前固定文本包已包装 gate=allow；不伪造历史2024许可或旧件来自2025 |
...
| 37 | <code>arxiv:2404.07738</code> | <code>materialized_sources/corpus/arxiv-2404.07738--3e68f613/manifest.yaml</code><br><code>raw_data/arxiv/ResearchAgent: Iterative Research Idea Generation over Scientific Literature with Large Language Models/metadata.yaml</code> | 固定 v2 与 33 个 source 同字节；NAACL2025 作者主文、附录及 16 表表达 BY4 路径已对应 | Vamathevan/FlyWire 独立摘要依据、两份 ACL 摘要包装、STY 真实差异及其他独立条件/NOTICE 尚待落实 | <code>documented_permission</code> | 满足具体剩余条件后再发布；当前 gate=block，unknown 不等于禁止 |
...
| 39 | <code>arxiv:2404.16130</code> | <code>materialized_sources/corpus/arxiv-2404.16130--fe9ed9e5/manifest.yaml</code><br><code>raw_data/arxiv/From Local to Global: A Graph RAG Approach to Query-Focused Summarization/metadata.yaml</code> | 固定v2论文BY4、当前模板对应表达BY4、AFaCTA独立归属；Fed有界研究引用不重许可为BY4 | 当前固定包已包装；19source/TeX/61selectors及正文原前缀保持，只附唯一说明 | <code>documented_permission</code> | 当前 gate=allow；限定当前科学方法演示，不授予新闻库或所有未来用途权利 |
...
| 45 | <code>arxiv:2408.06292</code> | <code>materialized_sources/corpus/arxiv-2408.06292--522d359c/manifest.yaml</code><br><code>raw_data/arxiv/The AI Scientist: Towards Fully Automated Open-Ended Scientific Discovery/metadata.yaml</code> | 固定 v3 与既有源包对应；论文 BY4、GDM BY-SA4、组件 LPPL 分别履约，完整 natbib 8.31 原 dtx 随 NOTICE 和 repo 保存 | 无；26源文件、60selectors、TeX与原正文前缀不变，仅追加唯一末注 | <code>documented_permission</code> | 当前固定文本包已包装 gate=allow；不等于人工批准或整体 PR 完成 |
| 46 | <code>arxiv:2408.08435</code> | <code>materialized_sources/corpus/arxiv-2408.08435--dc6e6730/manifest.yaml</code><br><code>raw_data/arxiv/Automated Design of Agentic Systems/metadata.yaml</code> | 固定 v2；论文与官方 ICLR2025 STY BY4，notation原作者使用说明、独立 LPPL及完整 natbib 原 dtx 分别保存 | 无；9源文件、33selectors、TeX与原93591-byte正文前缀不变，仅追加唯一末注 | <code>documented_permission</code> | 当前固定文本包已包装 gate=allow；不外推整个归档、图像或外链代码 |
...
| 53 | <code>arxiv:2411.14199</code> | <code>materialized_sources/corpus/arxiv-2411.14199--e07b84a9/manifest.yaml</code><br><code>raw_data/arxiv/OpenScholar: Synthesizing Scientific Literature with Retrieval-augmented LMs/metadata.yaml</code> | 固定 v1；论文 BY4、实际STY对应当前2025公开BY4、五篇ACL摘要BY4与独立LPPL/原dtx分别履约 | 无；16源文件、110selectors、TeX与原正文前缀不变，既存注释/页眉差异已披露 | <code>documented_permission</code> | 当前固定文本包已包装 gate=allow；不把文章许可或当前模板授权外推全部引用作品 |
| 54 | <code>arxiv:2501.04227</code> | <code>materialized_sources/corpus/arxiv-2501.04227--a0515b2c/manifest.yaml</code><br><code>raw_data/arxiv/Agent Laboratory: Using LLM Agents as Research Assistants/metadata.yaml</code> | 固定 v2 与既有源包对应；论文 BY4、GDM BY-SA4、组件 LPPL 分别履约，完整 natbib 8.31 原 dtx 随 NOTICE 和 repo 保存 | 无；8源文件、84selectors、TeX与原正文前缀不变，仅追加唯一末注 | <code>documented_permission</code> | 当前固定文本包已包装 gate=allow；不等于人工批准或整体 PR 完成 |
...
| 62 | <code>arxiv:2504.08066</code> | <code>materialized_sources/corpus/arxiv-2504.08066--22ed15f2/manifest.yaml</code><br><code>raw_data/arxiv/The AI Scientist-v2: Workshop-Level Automated Scientific Discovery via Agentic Tree Search/metadata.yaml</code> | 固定 v1 与既有源包对应；论文 BY4、GDM BY-SA4、OpenReview评审归属与组件 LPPL 分层包装，完整 natbib 原 dtx 随包保存 | 无；17源文件、78selectors、TeX与原正文前缀不变，七份评审未删除或去匿名 | <code>documented_permission</code> | 当前固定文本包已包装 gate=allow；不虚称七份评审均有同样纳入同意或人工批准 |
...
| 64 | <code>arxiv:2505.22954</code> | <code>materialized_sources/corpus/arxiv-2505.22954--8a7041cb/manifest.yaml</code><br><code>raw_data/arxiv/Darwin Godel Machine: Open-Ended Evolution of Self-Improving Agents/metadata.yaml</code> | 固定 v3；论文 BY4、ICLR模板当前2025 BY4表达/改编路径与独立LPPL条件已落实，完整原dtx随包及repo保存 | 无；34源文件、77selectors、TeX与原190099-byte正文前缀不变，年号及l→c差异披露 | <code>documented_permission</code> | 当前固定文本包已包装 gate=allow；不伪造独立2026许可、修改者或整体发布批准 |
...
| 74 | <code>methodology:linkml-schema-first</code> | <code>materialized_sources/corpus/methodology-linkml-schema-first--182a2b38/manifest.yaml</code><br><code>raw_data/methodology/LinkML Schema First Knowledge Modeling/metadata.yaml</code> | Apache-2.0 AND MIT；作者文档与实际 Furo 2025.12.19 内嵌模板分范围履约，旧文字包历史保全 | 仅 dated 单页；不声称 exact deployment commit、整站或图标全部上游链保证 | <code>documented_permission</code> | scoped 当前包 gate=allow；原 UID 总体边界仍 unresolved，最终验证见 PR26 报告 |
...
| 91 | <code>vocabulary:schema.org</code> | <code>materialized_sources/corpus/vocabulary-schema.org--c8b3dc95/manifest.yaml</code><br><code>raw_data/standard/Schema.org Vocabulary/metadata.yaml</code> | CC-BY-SA-3.0 AND Apache-2.0；dated 文档／内嵌模板分范围履约，gzip wire 与 entity 分别绑定，旧快照保全 | moving URL 的未来变化不继承本包；不代表完整 Vocabulary／release | <code>documented_permission</code> | scoped 当前包 gate=allow；原 UID 总体边界仍 unresolved，最终验证见 PR26 报告 |
